package org.firstinspires.ftc.teamcode.auto;

import com.acmerobotics.dashboard.config.Config;

@Config
public class PoseStorage {
    public static boolean useCamera = true;

    public static double startPosX = -36, startPosY = 65.9, startPosAngle = 0;
    public static double startConeStrafe1 = 54.8, startConeStrafe2 = 15, startConeForward = 5.05;
    public static double goToPark  = 52;

    public static double backIntakeOffset  = 13;


    public static double intakePose1XFirstCone = 38, intakePose1YFirstCone = -15;
    public static double intakePose2XFirstCone = 46, intakePose2YFirstCone = -15.6;


    public static double intakePoseCycleXSecondCone = 38, intakePose1YSecondCone = -14;
    public static double intakePose2XSecondCone = 47, intakePose2YSecondCone = -14.5;


    public static double intakePoseCycleXThirdCone = 38, intakePose1YThirdCone = -14.5;
    public static double intakePose2XThirdCone = 48, intakePose2YThirdCone = -14;


    public static double intakeAngle = 180;


    public static double posCone1X = 40, posCone1Y = -15, posConeAngle = 225;


    public static double posCone2XFirstCone = 32.45, posCone2YFirstCone = -18.05;


    public static double posCone2XSecondCone = 33.8, posCone2YSecondCone = -17.25;


    public static double posCone2XThirdCone = 33.8, posCone2YThirdCone = -17.5;


    public static double parkPoseX = 39.3, parkPoseY = -15, parkPoseAngle = 180;


    public static double TARGET_ZONE = 18, GO_TO_PARK_HELPER = 8;


    public static double BACK_WAIT_TIME = 0.1, DELIVERY_WAIT_TIME = .25, RELEASE_WAIT_TIME = .33;
    public static double ALMOST_RELEASE_TIME = 0.1, INTAKE_WAIT_TIME = .1, ELEVATOR_WAIT_TIME = .5;

//    public static double startPosX = -36;
//    public static double startPosY = 65.9;
//    public static double startPosAngle = 0;
//    public static double startConeStrafe1 = 58.8, startConeStrafe2 = 20, startConeForward = 4.65;
//
//
//    public static double backIntakeOffset  = 13;
//
//
//    public static double intakePose1XFirstCone = 33.1, intakePose1YFirstCone = -19.1;
//    public static double intakePose2XFirstCone = 47, intakePose2YFirstCone = -14.4;
//
//
//    public static double intakePoseCycleXSecondCone = 38, intakePose1YSecondCone = -11.8;
//    public static double intakePose2XSecondCone = 47.9, intakePose2YSecondCone = -11;
//
//
//    public static double intakePoseCycleXThirdCone = 38, intakePose1YThirdCone = -10.6;
//    public static double intakePose2XThirdCone = 49.5, intakePose2YThirdCone = -10;
//
//
//    public static double intakeAngle = 180;
//
//
//    public static double posCone1X = 42.1, posCone1Y = -14.2, posConeAngle = 225;
//
//
//    public static double posCone2XFirstCone = 33, posCone2YFirstCone = -19.2;
//
//
//    public static double posCone2XSecondCone = 34, posCone2YSecondCone = -18.9 ;
//
//    public static double posCone2XThirdCone = 34.75, posCone2YThirdCone = -18;
//
//
//    public static double parkPoseX = 40.2, parkPoseY = -11.5, parkPoseAngle = 180;
//
//
//    public static double TARGET_ZONE = 20, GO_TO_PARK_HELPER = 8;
//
//
//    public static double BACK_WAIT_TIME = 0.1, DELIVERY_WAIT_TIME = .35, RELEASE_WAIT_TIME = .33;
//    public static double ALMOST_RELEASE_TIME = 0.1, INTAKE_WAIT_TIME = .1, ELEVATOR_WAIT_TIME = .5;
}
